...
const hosts = {
  codex: { label: 'Codex', installed: '.agents/skills/glue' },
...
async function main() {
  ...
  if (Number(process.versions.node.split('.')[0]) < 22)
    fail('Glue needs Node.js 22 or later. This is Node ' + process.versions.node + '.');
  ...
  if (!inside || (!inside.startsWith('..') && !isAbsolute(inside)))
    fail(
      'Run this from inside the project that should get Glue, not from the Glue source folder:\n  cd YOUR_PROJECT\n  node "' +
  ...
  const ask = async (question, fallback) =>
    prompt ? (await prompt.question(question)).trim() || fallback : fallback;
  try {
    if (host === 'claude-desktop')
      fail(
        'Claude Desktop uses the Glue extension instead: open the .mcpb file with Claude Desktop and pick this folder.',
      );
    if (host === undefined) {
      const guess =
        existsSync(join(project, '.claude')) || existsSync(join(project, '.mcp.json'))
          ? 'claude-code'
          : 'codex';
      if (!interactive) host = guess;
        ...
        const answer = (
          await ask(
            'Which assistant will use Glue here? [1] Codex  [2] Claude Code  (default ' +
              (guess === 'codex' ? 1 : 2) +
              '): ',
            '',
          )
        ...
        host =
          answer === '1' || answer === 'codex'
            ? 'codex'
            : answer === '2' || answer.startsWith('claude')
              ? 'claude-code'
              : guess;
      }
    }
    if (!hosts[host])
      fail('Choose --host codex or --host claude-code. Claude Desktop uses the .mcpb extension.');
    console.log('\nProject: ' + project + '\nHost:    ' + hosts[host].label);
    if (interactive && !/^y(es)?$/i.test(await ask('Install Glue into this project? (y/n): ', 'n')))
      fail('Nothing was changed.');

    if (
      !existsSync(join(source, 'node_modules', 'zod')) ||
      !existsSync(join(source, 'node_modules', 'typescript'))
    )
      fail(
        'Glue\'s dependencies are not installed yet. Run this once, then try again:\n  cd "' +
          source +
          '"\n  npm ci',
      );
    const { buildInputs } = await import('./scripts/build-inputs.mjs');
    const built = existsSync(join(source, 'dist', 'build.json'))
      ? JSON.parse(readFileSync(join(source, 'dist', 'build.json'), 'utf8')).sourceId
      : undefined;
    ...
    if (install.status !== 0) {
      const reason =
        (install.stderr || install.stdout)
          ...
          ?.replace(/^.*Error: /, '') ?? 'Installation failed.';
      fail(
        reason +
          '\nNothing was installed.' +
          (/occupied|already/i.test(reason)
            ? ' Glue may already be set up in this project; see "Update or remove" in the README.'
            : ''),
      );
    }
    ...
    if (check.status !== 0 || !result?.ok)
      fail(
        'Glue was installed but its check failed: ' +
          (result?.error ?? check.stderr ?? 'unknown error'),
      );
    console.log(
      '\nGlue ' +
        result.version +
        ...
        ' tools.\nNext:\n  1. ' +
        (host === 'codex'
          ? 'Open this project in Codex and trust it when asked.'
          : 'Open this project in Claude Code and approve the "glue" server when asked.') +
