  ...
  while (true) {
    try {
      if (lstatSync(current).isSymbolicLink())
        throw new Error('Glue storage files and their parent directories must not be linked.');
    } catch (error) {
...
export function acquireLock(directory: string, name: string, message: string) {
    ...
    const detail =
      code === 'EEXIST'
        ...
        : code === 'EACCES' || code === 'EPERM'
          ...
          : 'Glue could not create its lock directory: ' +
            (error instanceof Error ? error.message : String(error));
    throw Object.assign(new Error(detail, { cause: error }), {
      code,
      path: lock,
      retryable: code === 'EEXIST',
    });
  }
  ...
  try {
    ...
    if (cleanupFailed && error instanceof Error)
      Object.assign(error, {
        lockNotReleased:
          'Glue could not clean up its lock after owner creation failed. Inspect the lock before another write.',
      });
    throw error;
  }
  let released = false;
  ...
  return (): string | undefined => {
    ...
    try {
      ...
      return (
        'Glue could not remove its lock (' +
...
export function withWriteLock<T>(
  directory: string,
  operation: () => T,
): { value: T; lockNotReleased?: string } {
  const release = acquireLock(
    directory,
    '.write-lock',
    'Glue store is busy. Retry the same request. Inspect .write-lock/owner.json and confirm a crashed writer stopped before removing its lock.',
  );
  let value: T;
  try {
    value = operation();
    ...
    if (failure && error instanceof Error)
      Object.assign(error, { lockNotReleased: failure + leftoverWriteLock });
    throw error;
  }
  const failure = release();
  return failure === undefined
    ? { value }
    : { value, lockNotReleased: failure + ' The work above is saved.' + leftoverWriteLock };
}
