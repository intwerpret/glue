...
export function lexicalPath(workspace: string, input: string) {
  ...
  if (!rel || rel.startsWith('../') || isAbsolute(rel) || /(^|\/)(\.git|\.codex|\.agents|\.claude|\.glue)(\/|$)/i.test(rel))
    throw Error('Path must stay in ordinary workspace files, outside .git, .codex, .agents, .claude and .glue.');
  if (/[. ](?:\/|$)/.test(rel) || rel.split('/').some(p => /^(con|prn|aux|nul|com\d|lpt\d)(\.|$)/i.test(p)))
...
export function assertContextPath(rel: string) {
  ...
  if (parts.some(part => part.startsWith('.')) ||
      /^(agents|claude|codex|gemini|skill|copilot-instructions)(\.[a-z0-9_-]+)?\.md$/i.test(parts.at(-1) ?? ''))
    throw Error('Context must be an ordinary Markdown note, outside dot-directories and not a host instruction file such as AGENTS.md, CLAUDE.md or SKILL.md.');
}
...
export function portableFile(workspace: string, input: string) {
  ...
  for (const part of parts) {
    ...
    if (existsSync(file)) {
      ...
      if (matches.length > 1) throw new PathCollision('Portable path collision. Resolve case or Unicode aliases explicitly; nothing was changed.');
      // A name that resolves without being listed is a filesystem alias, such as a Windows short name.
      if (!matches.length && existsSync(join(file, part))) throw Error('Reserved or ambiguous path.');
...
export function contextIdentity(
  owner: object, root: string, canonical: string, spellings: string[],
  hash: (value: string) => string, load: (directory: string) => string,
) {
  assertUnlinked(root);
  const ids = existsSync(root) ? readdirSync(root).filter(id => /^[a-f0-9]{64}$/.test(id)) : [];
  if (ids.length > 10000) throw Error('Context identity lookup exceeds 10000 entries. No identity claim was made.');
  const present = new Set(ids), cache = identityCache.get(owner) ?? new Map<string, string>();
  ...
  if (matches.size > 1) throw Error('Portable context identity collision. Preserve both histories and resolve explicitly.');
  const existing = [...matches][0];
  if (existing !== undefined) return existing;
  // An unreadable identity saved under another spelling might be this context. Never fork it implicitly.
  if (unavailable) throw Error('An unavailable saved identity prevents safely creating a new context. Inspect history first.');
  return canonical;
}
