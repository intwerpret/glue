...
import { assertContextPath, contextIdentity, lexicalPath, pathKey, portableFile, PathCollision } from './identity.js';
import { captureInput, savedCapture, prepareCapture, importedSchema, isSensitive, contentHash } from './captures.js';
...
const commitInput = resumeInput.extend({
  expectedVersion: digest.nullable().describe('null: create only if this context does not already exist. Otherwise the current saved version from resume; a stale value is a conflict and nothing is saved.'),
  markdown: z.string().min(1).max(32768).describe('Full replacement handoff text (UTF-8, at most 32 KiB).'),
  evidence: z.array(z.string().min(1).max(1000)).max(64).optional().describe('Workspace-relative files to snapshot. Omit to keep the current selection; [] clears it.'),
  reviewedEvidence: z.array(z.object({ path: z.string().min(1).max(1000), hash: digest }).strict()).max(64).optional().describe('Current hashes (from resume) of selected files that changed since the last save, acknowledging review.'),
  sensitiveEvidence: z.array(z.object({ path: z.string().min(1).max(1000), hash: digest }).strict()).max(64).optional().describe('Exact-content exceptions for selected files that trip the sensitive-content heuristic.'),
  captures: z.array(captureInput).max(16).optional().describe('Host-retrieved material as canonical base64 with declared representation/basis/scope. Omit to keep current captures; [] clears them. transfer:"allowed" only permits later selection for transfer.'),
  imported: importedSchema.optional(),
  record: recordSchema.nullable().optional().describe('Caller-declared metadata: title, kind, scope, provenance, status. Omit to keep; null clears.'),
  dependsOn: z.array(referenceSchema).max(32).optional().describe('Exact pins {context, version} of other contexts at their current heads. Omit to keep; [] clears.'),
  workingCopyHash: digest.nullable().optional().describe('Observed working-copy hash from resume when the Markdown file was edited outside Glue.') }).strict();
export const saveInput = commitInput.omit({ imported: true });
const evidenceSchema = z.object({ path: z.string(), hash: digest, bytes: z.number().int().nonnegative(), snapshot: z.string() }).strict();
const revisionSchema = z.object({ format: z.literal(1), context: z.string(), parent: digest.nullable(), request: digest,
  at: z.string(), markdown: z.string(), evidence: z.array(evidenceSchema),
  record: recordSchema.nullable().optional(), dependsOn: z.array(referenceSchema).max(32).optional(),
  captures: z.array(savedCapture).max(16).optional(), imported: importedSchema.optional(),
  workingCopyBefore: z.string().optional(), restoredFrom: digest.optional() }).strict();
...
function stored<T>(schema: z.ZodType<T>, bytes: Buffer, name: string): T {
  try { return schema.parse(JSON.parse(bytes.toString('utf8'))); }
  catch { throw Error('Saved Glue data is damaged: ' + name + ' is not valid. Use explicit recovery; history was not reset.'); }
}
...
export class Handoffs {
  ...
  constructor(workspace: string) { this.workspace = realpathSync(workspace); }
  resolvePath(input: string, context = false) {
    const path = lexicalPath(this.workspace, input);
    if (context && extname(path.file).toLowerCase() !== '.md') throw Error('Context must name a Markdown file.');
  ...
  location(context: string) {
    const path = this.path(context, true), root = join(this.workspace, '.glue', 'contexts');
  ...
  observeWorking(file: string) {
    try { return { available: true as const, ...this.working(file) }; }
    catch (error) { return { available: false as const, error: error instanceof Error ? error.message : 'Working copy could not be read.' }; }
  }
  ...
  verifyEvidence(directory: string, evidence: Revision['evidence'], checked?: Map<string, { hash: string; bytes: number }>) {
    for (const item of evidence) {
      ...
      if (actual.bytes !== item.bytes || actual.hash !== item.hash) throw Error('Evidence snapshot integrity failure: ' + item.path);
    }
  }
  ...
  private snapshotFile(directory: string, item: Revision['evidence'][number]) {
    ...
    if (!/^[a-f0-9]{64}(\.[a-z0-9]{1,12})?$/.test(item.snapshot) || !item.snapshot.startsWith(item.hash)) throw Error('Invalid snapshot path.');
  ...
  evidenceBytes(directory: string, item: Revision['evidence'][number]) {
    ...
    if (bytes.length !== item.bytes || hash(bytes) !== item.hash) throw Error('Evidence snapshot integrity failure: ' + item.path);
    return bytes;
  }
  committed(context: string, version?: string) {
    ...
    while (current) {
      ...
      if (visited.size >= 10000) throw Error('History lookup exceeds 10000 revisions; no committed membership claim was made.');
      visited.add(current);
      const saved = this.revision(head.directory, current);
      if (saved.context !== head.rel) throw Error('History context mismatch.');
      if (current === target) return { ...head, saved, version: target, currentVersion: head.version };
  ...
  references(context: string, input: Reference[]) {
    ...
    return input.map(ref => {
      ...
      if (path === context || seen.has(path)) throw Error('Dependencies must name distinct other contexts.');
      seen.add(path);
      const target = this.head(path);
      if (!target.saved || target.version !== ref.version) throw Error('Dependency changed or missing: ' + path + '. Read and reconcile before pinning it.');
  ...
  resume(input: unknown) {
    const { context } = resumeInput.parse(input), head = this.head(context);
    if (head.saved) {
    ...
    const evidence = (head.saved?.evidence ?? []).map(item => {
      let status = 'missing', currentHash: string | null = null, reason: 'path_collision' | undefined;
      try { const path = this.path(item.path).file; currentHash = hash(read(path)); status = currentHash === item.hash ? 'unchanged' : 'changed'; }
      catch (error) {
        if ((error as NodeJS.ErrnoException).code !== 'ENOENT') status = 'unavailable';
        if (error instanceof PathCollision) reason = 'path_collision';
      }
      return { path: item.path, status, ...(reason ? { reason } : {}), savedHash: item.hash, currentHash, snapshot: join(head.directory, 'evidence', item.snapshot) };
    });
    return { context: head.rel, version: head.version, markdown: head.saved?.markdown ?? null,
      workingCopy: { path: head.file, ...(working.available
        ? { hash: working.hash, status: working.hash === null ? 'missing' : head.saved && working.text === head.saved.markdown ? 'matches' : 'edited' }
        : { status: 'unavailable', error: working.error }) },
      evidence, history: join(head.directory, 'revisions'),
      captures: head.saved?.captures?.map(({ snapshot, ...item }) => ({ ...item, originalFreshness: 'not_checked' })) ?? [],
      imported: head.saved?.imported ?? null };
  }
  save(input: unknown, imported?: z.infer<typeof importedSchema>) {
    const request = commitInput.parse({ ...saveInput.parse(input), ...(imported ? { imported } : {}) }), loc = this.location(request.context);
    if (Buffer.byteLength(request.markdown) > 32768) throw Error('Keep the handoff within 32 KiB; reference full artifacts.');
    const selected = request.evidence === undefined ? undefined : [...new Set(request.evidence.map(p => this.resolvePath(p).rel))].sort();
    if (selected?.includes(pathKey(loc.rel))) throw Error('The handoff is already versioned; do not select it as evidence.');
    const captureIds = request.captures?.map(item => item.id) ?? [];
    if (new Set(captureIds).size !== captureIds.length) throw Error('Capture IDs must be distinct.');
    const suppliedCaptures = request.captures?.map(item => prepareCapture(item, new Date().toISOString()));
    ...
    try {
      const { value, lockNotReleased } = withWriteLock(loc.directory, () => this.commit(request, loc, selected, suppliedCaptures, key));
      ...
      if (created) { try { rmdirSync(loc.directory); } catch { /* preserved */ } }
      throw error;
    }
  }
  private commit(request: z.infer<typeof commitInput>, loc: ReturnType<Handoffs['location']>, selected: string[] | undefined,
    ...
    if (request.expectedVersion !== head.version) {
      ...
      let ancestor = head.version; const visited = new Set<string>();
      while (ancestor) {
        if (visited.has(ancestor)) throw Error('History cycle.'); visited.add(ancestor);
        const old = this.revision(loc.directory, ancestor);
        if (old.context !== loc.rel) throw Error('History context mismatch.');
        if (visited.size > 10000) throw Error('Retry lookup exceeds 10000 revisions. Inspect history; no new save was made.');
        if (old.request === key) { this.verifyEvidence(loc.directory, old.evidence); this.verifyCaptures(loc.directory, old.captures ?? []); return { committed: true, version: ancestor, currentVersion: head.version, replayed: true, workingCopyUpdated: false }; }
        ancestor = old.parent;
      }
    ...
    if ((request.workingCopyHash !== undefined && request.workingCopyHash !== working.hash) ||
        (diverged && request.workingCopyHash === undefined)) throw Error('Working copy changed. Resume, read the file, and supply its observed workingCopyHash to reconcile explicitly.');
    const paths = selected ?? head.saved?.evidence.map(e => this.resolvePath(e.path).rel) ?? [];
    const dependsOn = request.dependsOn === undefined ? head.saved?.dependsOn ?? [] : this.references(loc.rel, request.dependsOn);
    ...
    for (const item of request.reviewedEvidence ?? []) {
      ...
      if (!paths.includes(path) || reviewed.has(path)) throw Error('Review must name each selected evidence path at most once: ' + item.path);
      reviewed.set(path, item.hash);
    }
    const previous = new Map(head.saved?.evidence.map(item => [pathKey(item.path), item.hash]) ?? []);
    const exceptions = new Map((request.sensitiveEvidence ?? []).map(item => [this.resolvePath(item.path).rel, item.hash]));
    if (exceptions.size !== (request.sensitiveEvidence?.length ?? 0) || [...exceptions.keys()].some(path => !paths.includes(path)))
    ...
    const captured = paths.map(path => {
      const file = this.path(path).file, bytes = read(file); total += bytes.length;
      if (total > 64 * 1024 * 1024) throw Error('Selected evidence and captures exceed 64 MiB.');
      const id = hash(bytes), ext = extname(path).toLowerCase();
      if (isSensitive(bytes, path) && exceptions.get(path) !== contentHash(bytes))
        throw Error('Selected evidence may contain sensitive material. Review and supply a scoped exact-content exception if authorized.');
      if (reviewed.has(path) && reviewed.get(path) !== id) throw Error('Evidence changed since review: ' + path + '. Resume and review again; nothing was saved.');
      if (previous.has(path) && previous.get(path) !== id && reviewed.get(path) !== id)
        throw Error('Evidence changed: ' + path + '. Resume, read and reconcile the source, then supply its currentHash in reviewedEvidence; nothing was saved.');
      return { bytes, item: { path, hash: id, bytes: bytes.length, snapshot: id + (/^\.[a-z0-9]{1,12}$/.test(ext) ? ext : '') } };
    });
    for (const name of ['revisions', 'evidence']) { assertUnlinked(join(loc.directory, name)); mkdirSync(join(loc.directory, name), { recursive: true }); }
    for (const capture of captured) immutable(join(loc.directory, 'evidence', capture.item.snapshot), capture.bytes);
    for (const capture of suppliedCaptures ?? []) immutable(join(loc.directory, 'evidence', capture.item.snapshot), capture.bytes);
    const inheritedImport = head.saved?.imported;
    const imported = request.imported ?? (inheritedImport ? { ...inheritedImport,
      localModified: inheritedImport.localModified || request.markdown !== head.saved?.markdown ||
        request.captures !== undefined || request.evidence !== undefined || request.dependsOn !== undefined || request.record !== undefined,
    } : undefined);
    const revision: Revision = { format: 1, context: loc.rel, parent: head.version, request: key, at: new Date().toISOString(),
      markdown: request.markdown, evidence: captured.map(c => c.item), record, dependsOn,
    ...
    const bytes = JSON.stringify(revision), version = hash(bytes);
    ...
    return { committed: true, version, replayed: false, workingCopyUpdated,
      ...(workingCopyUpdated ? {} : { next: 'Resume and reconcile the working copy. The saved revision is intact.' }) };
  }
  inspect(context: string) {
    const loc = this.location(context);
    const parents = new Map<string, string | null>();
    const checked = new Map<string, { hash: string; bytes: number }>();
    const revisions: Array<{ version: string; valid: boolean; at?: string; parent?: string | null }> = [];
    ...
    const inventory = new Set(revisions.map(r => r.version)), reachable = new Set<string>();
    const missingParents = [...parents].flatMap(([version, parent]) => parent && !inventory.has(parent) ? [{ version, parent }] : []);
    const headBytes = existsSync(loc.head) ? read(loc.head) : null;
    let headVersion: string | null = null, status: 'complete' | 'incomplete' | 'unavailable' | 'empty' = 'empty';
    let reason: string | undefined, blockedAt: string | undefined;
    if (headBytes) {
      try { headVersion = headSchema.parse(JSON.parse(headBytes.toString('utf8'))).version; }
      catch { status = 'unavailable'; reason = 'invalid_head'; }
    } else if (revisions.length || existsSync(join(loc.directory, '.initialized'))) {
      status = 'unavailable'; reason = 'missing_head';
    }
    if (headVersion) {
      status = 'complete';
      ...
      while (current) {
        if (reachable.has(current) || !parents.has(current)) {
          status = 'incomplete'; blockedAt = current;
          reason = reachable.has(current) ? 'cycle' : inventory.has(current) ? 'invalid_revision' : 'missing_revision';
          if (inventory.has(current)) reachable.add(current);
          break;
        }
        reachable.add(current); current = parents.get(current)!;
      }
    }
    const working = this.observeWorking(loc.file);
    return { context: loc.rel, headHash: headBytes ? hash(headBytes) : null,
      ...(working.available ? { workingCopyHash: working.hash } : { workingCopy: { status: 'unavailable', error: working.error } }),
      revisions: revisions.map(r => ({ ...r, reachableFromHead: reachable.has(r.version) ? true : status === 'complete' || status === 'empty' ? false : null })),
  ...
  restore(context: string, version: string, expectedHeadHash: string | null, workingCopyHash: string | null) {
    digest.parse(version); const loc = this.location(context);
    const { value, lockNotReleased } = withWriteLock(loc.directory, () => {
      const headBytes = existsSync(loc.head) ? read(loc.head) : null;
      const working = this.working(loc.file);
      if ((headBytes ? hash(headBytes) : null) !== expectedHeadHash || working.hash !== workingCopyHash) throw Error('Recovery inputs changed. Inspect again.');
      ...
      const recovery = JSON.stringify({ head: headBytes?.toString('base64') ?? null, workingCopy: working.text, selected: version });
