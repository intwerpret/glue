...
export async function serveMcp(workspace: string) {
  ...
  const tools = [
    { name: 'glue_resume', description: 'Read a saved handoff or knowledge record and check selected evidence plus transitive declared dependencies. Reports affected records and gaps. Read-only.', schema: conditionalResumeInput, run: (input: unknown) => knowledge.resume(input) },
    { name: 'glue_checkpoint', description: 'Save Markdown, selected evidence, optional record metadata and exact dependsOn context/version pins. Omitted fields retain their prior values; [] clears selections/dependencies. Changed selected files require reviewedEvidence from resume after reconciliation. Metadata is caller-declared, not approval.', schema: saveInput, run: (input: unknown) => store.save(input) },
    { name: 'glue_find', description: 'Find saved handoffs, knowledge records and selected evidence using lexical search. Empty query lists records. Compact listings or concise matching evidence by default; full detail is available. Optional history and cursor. Returns exact references and coverage gaps; freshness is not checked.', schema: findInput, run: (input: unknown) => knowledge.find(input) },
    { name: 'glue_read', description: 'Fetch bounded exact Markdown or a selected source snapshot from a committed context version. Byte offsets, hashes and base64 preserve exact content. Follow parent for history. Does not certify live freshness; resume checks dependencies.', schema: readInput, run: (input: unknown) => knowledge.read(input) },
  ...
  for await (const line of requests(process.stdin, abort.signal)) {
    ...
    if (req.method === 'tools/list') {
      ...
      await send({ jsonrpc: '2.0', id, result: { tools: tools.map(tool => ({
        ...
        annotations: { readOnlyHint: tool.name !== 'glue_checkpoint' && tool.name !== 'glue_transfer', idempotentHint: true },
