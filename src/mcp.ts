...
export async function serveMcp(workspace: string) {
  ...
  const tools = [
    { name: 'glue_resume', description: 'Read a saved handoff or knowledge record and check selected evidence plus transitive declared dependencies. Reports affected records and gaps. For continuation, orient the user to the current goal, decisions, unfinished work, material changes and next action. Record status is not task completion; incomplete checks leave support unresolved. Read-only.', schema: conditionalResumeInput, run: (input: unknown) => knowledge.resume(input) },
    ...
    { name: 'glue_find', description: 'Find saved handoffs, knowledge records and selected evidence using lexical search. Empty query lists records. Compact listings or concise matching evidence by default; full detail is available. Optional history and cursor. Returns exact references and coverage gaps; freshness is not checked. Empty results or next:null with skipped bodies/gaps do not establish absence. Resume likely contexts to identify selected sources, then read the relevant saved bytes.', schema: findInput, run: (input: unknown) => knowledge.find(input) },
