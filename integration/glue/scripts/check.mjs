...
try {
  ...
  if (
    identity.format !== 1 ||
    fingerprint(identity.files) !== identity.packageId ||
    fingerprint(actual) !== identity.packageId
  )
    throw Error('Installed package differs from its recorded build.');
  const binding = existsSync(join(installation, 'connection.json'))
    ? JSON.parse(readFileSync(join(installation, 'connection.json'), 'utf8'))
    : { format: 1, host: 'codex', name: 'glue', config: join(workspace, '.codex/config.toml') };
  if (
    binding.format !== 1 ||
    !['codex', 'claude-code', 'claude-desktop'].includes(binding.host) ||
  ...
  const parsed =
    binding.host === 'codex'
      ? parseConfiguration(readConfigurationText(config))
      : parseJsonConfiguration(readConfigurationText(config));
    ...
    [
      {
        ...
        params: {
          ...
          capabilities: {},
          clientInfo: { name: 'glue-check', version: '1.0' },
        },
      },
  ...
  if (
    ...
    initialized?.result?.serverInfo?.version !== identity.version
  )
    throw Error('MCP initialization or product version verification failed.');
