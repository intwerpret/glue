...
function fixture(t) {
  const dir = fs.mkdtempSync(join(fs.realpathSync(tmpdir()), 'glue-atomic-test-'));
...
for (const failure of ['writeFileSync', 'renameSync'])
  test('atomic write cleans its own temporary file after ' + failure + ' fails', t => {
    ...
    fs.writeFileSync(target + '.tmp', 'unrelated');
...
test('failed exclusive temporary creation preserves the colliding file', t => {
  ...
  try {
    fs.openSync = (file, flags, ...args) => {
      ...
      collision = file;
      const fd = original(file, 'wx', 0o600);
  ...
  assert.equal(fs.readFileSync(collision, 'utf8'), 'Other owner');
  assert.equal(fs.readFileSync(target, 'utf8'), 'original');
});
