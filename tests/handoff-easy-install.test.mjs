...
const run = (cwd, args = []) => spawnSync(process.execPath, [installer, ...args], { cwd, input: '', encoding: 'utf8', windowsHide: true, timeout: 120000 });

test('one command from inside a project installs for the detected host, ignores Glue files in Git and passes the check', t => {
  for (const [marker, directory, ignored] of [[null, '.agents/skills/glue', '.codex/config.toml'], ['.claude', '.claude/skills/glue', '.mcp.json']]) {
    const project = fixture(t);
    assert.equal(spawnSync('git', ['init', '--quiet', project], { encoding: 'utf8', windowsHide: true }).status, 0);
    ...
    for (const line of ['node_modules/', '.glue/', directory + '/', ignored]) assert.ok(lines.includes(line), line);
    const again = run(project);
    assert.notEqual(again.status, 0);
    assert.match(again.stderr, /Nothing was installed/);
    assert.equal(readFileSync(join(project, '.gitignore'), 'utf8').split('\n').filter(line => line === '.glue/').length, 1);
  }
});
...
test('the easy installer refuses the Glue source folder, a missing folder and Claude Desktop without changing anything', t => {
  ...
  assert.notEqual(inSource.status, 0); assert.match(inSource.stderr, /not from the Glue source folder/);
  const missing = run(project, [join(project, 'missing')]);
  assert.notEqual(missing.status, 0); assert.match(missing.stderr, /does not exist/);
  const desktop = run(project, ['--host', 'claude-desktop']);
  assert.notEqual(desktop.status, 0); assert.match(desktop.stderr, /\.mcpb/);
...
test('invalid Git exclusion target refuses both hosts before activation and permits retry after repair', t => {
  for (const host of ['codex', 'claude-code']) {
    const project = fixture(t); git(project, ['init', '--quiet']);
    mkdirSync(join(project, '.gitignore'));
    mkdirSync(join(project, '.glue')); writeFileSync(join(project, '.glue', 'existing'), 'preserve saved bytes');
...
test('tracked Glue files stop installation without disclosing names or changing tracking', t => {
  ...
  assert.notEqual(child.status, 0); assert.match(child.stderr, /already tracks/);
...
test('nested Git projects and later negation rules get effective exclusions without showing existing contents', t => {
  const repository = fixture(t); git(repository, ['init', '--quiet']);
  const project = join(repository, 'nested'); mkdirSync(project);
...
test('exclusion write errors and exclusions changed before connection activation roll back only the new package', t => {
  for (const host of ['codex', 'claude-code']) for (const failure of ['write', 'activation']) {
    const project = fixture(t); git(project, ['init', '--quiet']);
    const directory = host === 'codex' ? '.agents/skills/glue' : '.claude/skills/glue';
    const config = host === 'codex' ? '.codex/config.toml' : '.mcp.json';
    if (host === 'codex') mkdirSync(join(project, '.codex'));
    const configBytes = host === 'codex' ? '# unrelated setting\n' : '{"mcpServers":{"other":{"command":"preserve"}}}\n';
    writeFileSync(join(project, config), configBytes);
    mkdirSync(join(project, '.glue')); writeFileSync(join(project, '.glue', 'existing'), 'unchanged');
    const harness = `
    ...
    `;
    const child = spawnSync(process.execPath, ['--input-type=module', '-e', harness, project, host, failure, directory], { cwd: resolve('.'), encoding: 'utf8', windowsHide: true, timeout: 30000 });
    ...
    assert.equal(readdirSync(project).some(name => name.startsWith('.glue-install-')), false);
  }
});
...
test('partial wildcard exclusions cannot leave individual saved files exposed', t => {
  const project=fixture(t);git(project,['init','--quiet']);
  mkdirSync(join(project,'.glue'));writeFileSync(join(project,'.glue','keep'),'preserved');
...
test('Git exclusion checks do not run commands or hooks configured by the project repository', async t => {
  ...
  const project = fixture(t); git(project, ['init', '--quiet']);
  const marker = join(project, 'executed'), script = join(project, 'monitor.cjs');
  ...
  writeFileSync(join(project, 'tracked.txt'), 'tracked'); git(project, ['add', 'tracked.txt']);
  ...
  if (!existsSync(marker)) { t.skip('this Git does not run a configured fsmonitor command'); return; }
  rmSync(marker);
