...
import { existsSync, mkdtempSync, realpathSync, readFileSync, readdirSync, rmSync, writeFileSync } from 'node:fs';
...
function assertAbsent(directory, marker) {
  for (const bytes of contents(directory)) {
    assert.equal(bytes.includes(Buffer.from(marker)), false, 'Private marker survived in generated data');
    assert.equal(bytes.includes(Buffer.from(Buffer.from(marker).toString('base64'))), false, 'Encoded private marker survived in generated data');
  }
}
const capture = (body, extra = {}) => ({
  id: 'source', label: 'Selected source', representation: 'original-bytes', basis: 'Selected bytes',
  scope: 'This synthetic project only', base64: Buffer.from(body).toString('base64'), ...extra,
});
const save = (store, extra = {}) => store.save({ context: 'handoff.md', expectedVersion: null, markdown: 'Public-safe fixture', ...extra });

test('blocked mixed captures leave no sensitive bytes in Glue data or diagnostic text', t => {
  const { workspace, store } = fixture(t), marker = 'SYNTHETIC_PRIVATE_CAPTURE_987654';
  const failure = () => save(store, { captures: [capture('safe text'), capture(marker, { id: 'credential', label: '.env' })] });
...
test('sensitive local-source exceptions cannot authorize siblings or changed source bytes', t => {
  const { workspace, store } = fixture(t), marker = 'SYNTHETIC_LOCAL_SECRET_123456';
  writeFileSync(join(workspace, '.env'), marker);
  writeFileSync(join(workspace, '.env.other'), marker + '_SIBLING');
  assert.throws(() => save(store, { evidence: ['.env', '.env.other'], sensitiveEvidence: [{ path: '.env', hash: hash(marker) }] }), /sensitive material/);
  assertAbsent(join(workspace, '.glue'), marker);
  const saved = save(store, { evidence: ['.env'], sensitiveEvidence: [{ path: '.env', hash: hash(marker) }] });
  const before = readFileSync(store.location('handoff.md').head);
  writeFileSync(join(workspace, '.env'), marker + '_CHANGED');
  assert.throws(() => store.save({ context: 'handoff.md', expectedVersion: saved.version, markdown: 'Must not commit',
    sensitiveEvidence: [{ path: '.env', hash: hash(marker) }], reviewedEvidence: [{ path: '.env', hash: hash(marker + '_CHANGED') }],
  }), /sensitive material/);
...
test('host capture checks remain attributed observations and do not adopt new bytes', t => {
  ...
  const saved = save(store, { captures: [capture('original bytes', { origin, retrievedAt: '2020-01-01T00:00:00.000Z' })] });
  ...
  const request = { context: 'handoff.md', capture: 'source', hash: hash('changed bytes'),
    representation: 'original-bytes', basis: 'Selected bytes', origin, retrievedAt: '2020-01-01T00:00:00.000Z' };
  ...
  assert.equal(knowledge.checkCapture({ ...request, basis: 'Different extraction' }).comparison, 'basis_changed');
...
test('binary capture paging preserves exact bytes and duplicate IDs do not commit', t => {
  const { workspace, store, knowledge } = fixture(t), bytes = Buffer.from([0, 255, 240, 159, 153, 130, 13, 10, 128, 1]);
  const request = { context: 'binary.md', expectedVersion: null, markdown: 'Binary source', captures: [capture(bytes)] };
  assert.throws(() => store.save({ ...request, captures: [capture(bytes), capture('other bytes')] }), /distinct/);
  assert.equal(store.resume({ context: 'binary.md' }).version, null);
  const saved = store.save(request), parts = [];
  let offset = 0;
  do {
    const page = knowledge.read({ context: 'binary.md', version: saved.version, capture: 'source', offset, limit: 3, representation: 'base64' });
...
test('capture selection omission retains, explicit clearing preserves historical bytes, and retries stay identical', t => {
  const { store, knowledge } = fixture(t);
  const request = { context: 'handoff.md', expectedVersion: null, markdown: 'First capture', captures: [capture('exact retained capture')] };
  ...
  const retained = store.save({ context: request.context, expectedVersion: first.version, markdown: 'Later progress' });
  assert.equal(knowledge.read({ context: request.context, capture: 'source' }).text, 'exact retained capture');
  const cleared = store.save({ context: request.context, expectedVersion: retained.version, markdown: 'Selection cleared', captures: [] });
  assert.deepEqual(knowledge.resume({ context: request.context }).captures, []);
  assert.throws(() => knowledge.read({ context: request.context, capture: 'source' }), /not selected/);
  assert.equal(knowledge.read({ context: request.context, version: first.version, capture: 'source' }).text, 'exact retained capture');
  const oldRetry = store.save(request);
  assert.equal(oldRetry.version, first.version); assert.equal(oldRetry.currentVersion, cleared.version);
...
test('corrupt capture snapshots fail resume and exact reads instead of returning unchecked bytes', t => {
  ...
  const loc = store.location('handoff.md'), item = store.head('handoff.md').saved.captures[0];
...
test('missing, saved and unreadable-working-copy resumes omit absolute workspace locators', t => {
  ...
  const check = view => {
    for (const value of strings(view)) {
      assert.equal(value.replaceAll('\\', '/').includes(workspace.replaceAll('\\', '/')), false, 'Response leaked the absolute workspace locator');
    }
  };
...
test('selected clean-store rebuild excludes old snapshots, manual text and recovery while preserving the original', t => {
  const source = fixture(t), target = fixture(t), marker = 'SYNTHETIC_RETIRED_HISTORY_654321';
  writeFileSync(join(source.workspace, 'old-evidence.txt'), marker + '_EVIDENCE');
  const old = save(source.store, { markdown: marker + '_MARKDOWN', evidence: ['old-evidence.txt'] });
  writeFileSync(join(source.workspace, 'handoff.md'), marker + '_MANUAL');
  const clean = source.store.save({ context: 'handoff.md', expectedVersion: old.version,
    markdown: 'Clean selected continuation', evidence: [], workingCopyHash: hash(marker + '_MANUAL'),
    captures: [capture('Reviewed selected support', { transfer: 'allowed', scope: 'Reusable synthetic example' })],
  });
  ...
  source.store.restore('handoff.md', clean.version, inspection.headHash, inspection.workingCopyHash);
  const loc = source.store.location('handoff.md');
  assert.equal(contents(loc.directory).some(bytes => bytes.includes(Buffer.from(marker))), true);
  const sourceHead = readFileSync(loc.head), sourceHistory = contents(loc.directory).map(bytes => hash(bytes)).sort();
  const transfer = new Transfers(source.store), receiving = new Transfers(target.store);
  const selection = { context: 'handoff.md', version: clean.version, captures: ['source'], evidence: [] };
  ...
  const exported = transfer.run({ action: 'export', ...selection, reviewedHash: preview.payloadHash });
  ...
  assert.throws(() => receiving.run({ action: 'import', bundle: exported.bundle, reviewedHash: '0'.repeat(64),
    context: 'rebuilt.md', expectedVersion: null }), /reviewed/);
  assert.equal(target.store.resume({ context: 'rebuilt.md' }).version, null);
  assert.deepEqual(readFileSync(loc.head), sourceHead);
  const imported = receiving.run({ action: 'import', bundle: exported.bundle, reviewedHash: importPreview.payloadHash,
    context: 'rebuilt.md', expectedVersion: null });
  assert.equal(imported.committed, true);
  assert.equal(target.knowledge.read({ context: 'rebuilt.md' }).text, 'Clean selected continuation');
  assert.equal(target.knowledge.read({ context: 'rebuilt.md', capture: 'source' }).text, 'Reviewed selected support');
  ...
  assert.deepEqual(readFileSync(loc.head), sourceHead);
  assert.deepEqual(contents(loc.directory).map(bytes => hash(bytes)).sort(), sourceHistory);
  assert.equal(contents(loc.directory).some(bytes => bytes.includes(Buffer.from(marker))), true);
});

test('JSON credentials under an innocuous capture label are refused without retaining the value', t => {
  const { workspace, store } = fixture(t), marker = 'SYNTHETIC_JSON_CREDENTIAL_112233';
  const body = JSON.stringify({ password: marker });
  assert.throws(() => save(store, { captures: [capture(body, { label: 'Ordinary document' })] }), error => {
    assert.match(error.message, /sensitive material/);
    assert.equal(error.message.includes(marker), false);
    return true;
  });
...
test('portable metadata screening stays fast on long adversarial runs', async () => {
  ...
  for (const run of ['a'.repeat(size), 'a.b-'.repeat(size / 4), 'x@'.repeat(size / 2), 'ftp//'.repeat(size / 5) + ' ' + '@a.'.repeat(size / 3)]) {
    ...
    try { assertPortableMetadata(run); } catch { /* only duration matters here */ }
