...
function fixture(t) {
  const workspace = mkdtempSync(join(fs.realpathSync(tmpdir()), 'glue-handoff-test-'));
...
test('MCP exposes six tools, leaves unrelated files untouched and rejects unknown actions', t => {
  ...
  writeFileSync(join(workspace, 'unrelated.txt'), 'unrelated content');
  const requests = [
    { id: 1, method: 'tools/list' },
    {
      id: 2,
      method: 'tools/call',
      params: { name: 'glue_resume', arguments: { context: 'handoff.md' } },
    },
    { id: 3, method: 'tools/call', params: { name: 'unknown_tool', arguments: {} } },
    {
      id: 4,
      method: 'tools/call',
      params: { name: 'glue_checkpoint', arguments: { ...first('MCP ✅') } },
    },
  ];
  const p = mcpRun(workspace, {
  ...
  assert.deepEqual(
    rows[0].result.tools.map(t => t.name),
    [
      'glue_resume',
      'glue_checkpoint',
      'glue_find',
  ...
  assert.ok(rows[0].result.tools.every(tool => tool.inputSchema.type === 'object'));
  assert.equal(
    rows[0].result.tools.find(tool => tool.name === 'glue_transfer').annotations.readOnlyHint,
    false,
  );
  assert.equal(JSON.parse(rows[1].result.content[0].text).version, null);
  assert.equal(rows[2].error.code, -32602);
  assert.equal(JSON.parse(rows[3].result.content[0].text).committed, true);
...
test('MCP rejects invalid UTF-8 requests without writes and continues with valid requests', t => {
  ...
  const input = Buffer.concat([
    malformed,
    Buffer.from(valid.map(r => JSON.stringify(r)).join('\n')),
  ]);
  ...
  assert.equal(rows[1].id, 'Unicode 🙂');
  assert.deepEqual(rows[1].result, {});
  const resumed = JSON.parse(rows[2].result.content[0].text);
  assert.equal(resumed.version, saved.version);
  assert.equal(resumed.markdown, request.markdown);
...
test('MCP rejects an incomplete UTF-8 sequence at EOF without crashing or writing', t => {
  const { workspace } = fixture(t);
  const p = mcpRun(workspace, {
    input: Buffer.from([0xe2, 0x82]),
    encoding: 'utf8',
    windowsHide: true,
  });
  assert.equal(p.status, 0, p.stderr);
  assert.equal(JSON.parse(p.stdout).error.code, -32700);
...
test('MCP refuses oversized requests before any write', t => {
  const { workspace } = fixture(t);
  const p = mcpRun(workspace, {
    input: Buffer.alloc(2 * 1024 * 1024 + 1, 32),
    encoding: 'utf8',
    windowsHide: true,
  });
  assert.equal(p.status, 0, p.stderr);
  assert.equal(JSON.parse(p.stdout.trim()).error.code, -32600);
  assert.deepEqual(readdirSync(workspace), []);
});
...
test('MCP delegates argument validation to core and rejects malformed calls without writes', t => {
  ...
  const cases = [
    ...
    ['glue_resume', { context: request.context, extra: true }],
    ['glue_checkpoint', {}],
    ['glue_checkpoint', { ...request, markdown: 42 }],
    ['glue_checkpoint', { ...request, expectedVersion: 42 }],
    ['glue_checkpoint', { ...request, extra: true }],
  ];
  cases.push(
    ['glue_find', null],
    ['glue_find', { limit: 0 }],
    ...
    ['glue_read', {}],
    ['glue_read', { context: request.context, offset: -1 }],
    ['glue_read', { context: request.context, extra: true }],
  );
  for (const [name, args] of cases) {
    if (name === 'glue_resume') assert.throws(() => store.resume(args));
    if (name === 'glue_checkpoint') assert.throws(() => store.save(args));
  }
  const requests = cases.map(([name, args], id) => ({
    jsonrpc: '2.0',
    id,
    method: 'tools/call',
  ...
  for (let i = 0; i < cases.length; i++) {
    assert.equal(rows[i].id, i);
    assert.equal(rows[i].result.isError, true);
  }
