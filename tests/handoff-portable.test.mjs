...
function legacy(workspace, context, markdown = 'Legacy selected work', working = true) {
  ...
  const bytes = JSON.stringify({ format: 1, context, parent: null, request: hash('legacy request ' + context), at: '2026-01-01T00:00:00.000Z', markdown, evidence: [] });
  const version = hash(bytes), revision = join(directory, 'revisions', version + '.json');
  ...
  if (working) { mkdirSync(dirname(join(workspace, context)), { recursive: true }); writeFileSync(join(workspace, context), markdown); }
...
test('portable context aliases preserve one identity and actual case/Unicode filename', t => {
  ...
  const context = 'Nótes/Café.MD', alias = 'no\u0301tes/cafe\u0301.md';
...
test('legacy mixed-case history survives relocation and alias saves without revision rewriting', t => {
  const original = fixture(t), moved = fixture(t);
  ...
  const receipt = store.save({ context: 'NOTES/DECISION.MD', expectedVersion: prior.version, markdown: 'Continued after relocation' });
  const location = store.location('notes/decision.md');
  assert.equal(location.directory, join(moved.workspace, '.glue', 'contexts', hash('Notes/Decision.MD')));
  assert.equal(store.revision(location.directory, receipt.version).parent, prior.version);
  assert.equal(readFileSync(join(location.directory, 'revisions', prior.version + '.json'), 'utf8'), prior.bytes);
  assert.equal(readFileSync(prior.revision, 'utf8'), prior.bytes);
  assert.equal(readFileSync(join(moved.workspace, 'Notes/Decision.MD'), 'utf8'), 'Continued after relocation');
...
test('adding a competing legacy identity is detected even after cached canonical lookup', t => {
  ...
  assert.throws(() => store.save({ context: 'note.md', expectedVersion: receipt.version, markdown: 'must not overwrite' }), /identity collision/);
...
test('unavailable unrelated identity does not hide known history but blocks unproven new identity', t => {
  ...
  assert.throws(() => store.save({ context: 'new.md', expectedVersion: null, markdown: 'Cannot establish identity' }), /unavailable saved identity/);
...
test('case-only filesystem rename does not fork a portable context', t => {
  const { workspace, store } = fixture(t);
  const receipt = store.save({ context: 'Note.md', expectedVersion: null, markdown: 'Before rename' });
...
test('distinct filesystem aliases are rejected on volumes that permit them', t => {
  ...
  try { writeFileSync(join(workspace, 'note.md'), 'Second', { flag: 'wx' }); }
  catch (error) { if (error.code === 'EEXIST') { assert.equal(store.path('note.md').file, join(workspace, 'Note.md')); return; } throw error; }
...
test('resume names a path collision as the reason selected evidence is unavailable', t => {
  ...
  const receipt = store.save({ context: 'note.md', expectedVersion: null, markdown: 'Pinned source', evidence: [composed] });
  store.save({ context: 'root.md', expectedVersion: null, markdown: 'Depends on the note', dependsOn: [{ context: 'note.md', version: receipt.version }] });
  assert.equal(store.resume({ context: 'note.md' }).evidence[0].status, 'unchanged');
  try { writeFileSync(join(workspace, decomposed), 'Decomposed', { flag: 'wx' }); }
  catch (error) {
    if (error.code === 'EEXIST') { t.skip('this volume folds Unicode aliases, so two such names cannot coexist'); return; }
    throw error;
  }
  ...
  const issue = { version: receipt.version, reason: 'live_source_path_collision', source: pathKey(composed) };
  assert.deepEqual(new Knowledge(store).resume({ context: 'note.md' }).basis.issues, [{ context: 'note.md', via: [], ...issue }]);
  assert.deepEqual(new Knowledge(store).resume({ context: 'root.md' }).basis.issues, [{ context: 'note.md', via: ['root.md'], ...issue }]);
});

test('Unicode normalization collisions are refused when distinct entries coexist', t => {
  const { workspace, store } = fixture(t);
  const composed = 'Caf\u00e9.md', decomposed = 'Cafe\u0301.md';
  writeFileSync(join(workspace, composed), 'Composed');
  try { writeFileSync(join(workspace, decomposed), 'Decomposed', { flag: 'wx' }); }
  catch (error) {
    if (error.code === 'EEXIST') { assert.equal(store.path(decomposed).rel, pathKey(composed)); return; }
    throw error;
  }
...
test('a failed first save leaves no historical identity that blocks unrelated new work', t => {
  const { workspace, store } = fixture(t);
  assert.throws(() => store.save({ context: 'failed.md', expectedVersion: null, markdown: 'No saved revision', evidence: ['missing.txt'] }));
  ...
  const receipt = store.save({ context: 'other.md', expectedVersion: null, markdown: 'Independent work' });
  ...
  assert.throws(() => store.save({ context: 'another.md', expectedVersion: null, markdown: 'Do not ignore an active writer' }), /unavailable saved identity/);
