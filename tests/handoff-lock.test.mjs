...
function withLockRemovalDenied(run,step='unlinkSync',code='EPERM'){
 ...
 try {
  fs[step]=(file,...args)=>{if(String(file).endsWith(target))throw Object.assign(new Error('delete denied'),{code});return original(file,...args);};syncBuiltinESMExports();
  return run();
 } finally {fs[step]=original;syncBuiltinESMExports();}
}
const first={context:'notes/a.md',expectedVersion:null,markdown:'Saved text',evidence:[]};

test('a write lock that cannot be removed does not hide a committed save',t=>{
 const workspace=fixture(t),store=new Handoffs(workspace);
 ...
 assert.equal(view.version,saved.version);assert.equal(view.markdown,'Saved text');
 assert.throws(()=>store.save({...first,expectedVersion:saved.version,markdown:'Second'}),error=>error.code==='EEXIST'&&error.retryable===true);
});
...
test('a restore reports a lock it could not remove',t=>{
 const workspace=fixture(t),store=new Handoffs(workspace),saved=store.save(first);
 store.save({...first,expectedVersion:saved.version,markdown:'Later text'});
 const inspection=store.inspect('notes/a.md');
 const restored=withLockRemovalDenied(()=>store.restore('notes/a.md',saved.version,inspection.headHash,inspection.workingCopyHash));
...
test('an operation failure is reported with the lock it left behind',t=>{
 ...
 assert.throws(()=>withLockRemovalDenied(()=>withWriteLock(workspace,()=>{throw failure;})),error=>error===failure&&/EPERM/.test(error.lockNotReleased)&&!/work above is saved/.test(error.lockNotReleased));
 const clean=new Error('operation failed');
 assert.throws(()=>withWriteLock(fixture(t),()=>{throw clean;}),error=>error===clean&&!('lockNotReleased' in error));
});

test('a rejected first save names the lock that now blocks new contexts',t=>{
 const workspace=fixture(t),store=new Handoffs(workspace);
 assert.throws(()=>withLockRemovalDenied(()=>store.save({...first,evidence:['missing.txt']})),error=>error.code==='ENOENT'&&/blocks creating any new context/.test(error.lockNotReleased));
 assert.throws(()=>store.save({...first,context:'notes/b.md'}),/unavailable saved identity/);
});
...
test('a namespace lock left by the first transfer does not block later transfers',t=>{
 const workspace=fixture(t),store=new Handoffs(workspace),transfers=new Transfers(store),saved=store.save(first);
 const selection={action:'preview',context:'notes/a.md',version:saved.version,captures:[],evidence:[]};
...
test('installation lock rejects overlap without disturbing its owner and remains separate from write locks',t=>{
 const workspace=fixture(t),owner=join(workspace,'.glue-install-lock/owner.json');
 assert.equal(withInstallationLock(workspace,()=>{
  const before=fs.readFileSync(owner);assert.equal(JSON.parse(before).pid,process.pid);
  assert.throws(()=>withInstallationLock(workspace,()=>assert.fail('overlapping operation ran')),error=>error.code==='EEXIST'&&error.retryable===true&&/Another installation operation/.test(error.message));
  assert.deepEqual(fs.readFileSync(owner),before);
  assert.deepEqual(withWriteLock(workspace,()=>42),{value:42});assert.ok(fs.existsSync(owner));
  return 'done';
 }),'done');
...
test('installation lock reports permission failures without claiming contention',t=>{
 const workspace=fixture(t),original=fs.mkdirSync,lock=join(workspace,'.glue-install-lock');
 try {
  for(const code of ['EACCES','EPERM']){
   fs.mkdirSync=(file,...args)=>{if(file===lock)throw Object.assign(new Error('denied'),{code});return original(file,...args);};syncBuiltinESMExports();
   assert.throws(()=>withInstallationLock(workspace,()=>assert.fail('operation ran without lock')),error=>error.code===code&&error.retryable===false&&error.path===lock&&/host write permissions/.test(error.message)&&!/Another installation/.test(error.message));
  }
