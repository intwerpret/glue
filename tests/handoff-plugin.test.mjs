...
function run(plugin, project, cwd, request) {
  ...
  return spawnSync(process.execPath, [join(plugin, 'server.mjs')], { cwd, env, encoding: 'utf8', windowsHide: true,
    input: [initialize, { jsonrpc: '2.0', method: 'notifications/initialized' }, request].map(JSON.stringify).join('\n') + '\n', timeout: 10000 });
}
...
test('plugin package has reviewed components, relocatable paths and verifiable inventory', t => {
  ...
  assert.deepEqual(files, receipt.files); assert.equal(fingerprint(files), receipt.packageId);
  assert.equal(Object.keys(files).some(p => /(^|\/)(\.git|\.glue|notes|history|knowledge)(\/|$)|\.map$/.test(p)), false);
  assert.ok(files['runtime/node_modules/zod/LICENSE']);
  for (const p of Object.keys(files)) assert.equal(readFileSync(join(plugin, p)).includes(Buffer.from(root)), false);
...
test('plugin refuses missing, relative, unavailable and package-overlapping project bindings', t => {
  const { plugin, project, root } = fixture(t); packageClaudeCode(plugin);
  for (const invalid of [undefined, '.', '${CLAUDE_PROJECT_DIR}', join(root, 'missing'), plugin, join(plugin, 'runtime'), root]) {
...
test('packaged server binds host project rather than cwd and survives a fresh process', t => {
  const { plugin, project, other } = fixture(t); packageClaudeCode(plugin);
  writeFileSync(join(other, 'private.txt'), 'UNSELECTED_OTHER_PROJECT_MARKER');
  const save = run(plugin, project, other, call('glue_checkpoint', { context: 'notes/lantern.md', expectedVersion: null, markdown: 'Lantern budget is 500.' }));
  ...
  const isolated = run(plugin, other, project, call('glue_resume', { context: 'notes/lantern.md' }));
  assert.equal(JSON.parse(JSON.parse(isolated.stdout.trim().split('\n').at(-1)).result.content[0].text).version, null);
  ...
  assert.equal(fingerprint(collectFiles(plugin)), JSON.parse(readFileSync(join(plugin, 'installation.json'))).packageId);
});

test('switching package directories retains project history and removing the package leaves saved work', t => {
  const { root, project, other, plugin } = fixture(t); packageClaudeCode(plugin);
  const saved = run(plugin, project, other, call('glue_checkpoint', { context: 'note.md', expectedVersion: null, markdown: 'Retained through package replacement.' }));
  assert.equal(saved.status, 0, saved.stderr);
  const before = collectFiles(project);
  const replacement = join(root, 'Replacement plugin'); packageClaudeCode(replacement);
  const resumed = run(replacement, project, other, call('glue_resume', { context: 'note.md' }));
  assert.equal(resumed.status, 0, resumed.stderr); assert.match(resumed.stdout, /Retained through package replacement/);
...
test('marketplace package lists the packaged plugin by relative source and ships its license', async t => {
  ...
  const manifest = JSON.parse(readFileSync(join(output, '.claude-plugin', 'marketplace.json'), 'utf8'));
