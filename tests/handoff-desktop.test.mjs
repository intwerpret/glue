...
function run(bundle, selected, cwd, call, hostArguments = []) {
  ...
  const env = { ...process.env, CLAUDE_PROJECT_DIR: cwd }; delete env.GLUE_PROJECT_DIR;
  if (selected !== undefined) env.GLUE_PROJECT_DIR = manifest.server.mcp_config.env.GLUE_PROJECT_DIR.replace('${user_config.project_folder}', selected);
  const args = manifest.server.mcp_config.args.map(x => x.replace('${__dirname}', bundle)).concat(hostArguments);
  return spawnSync(process.execPath, args, { cwd, env, windowsHide: true, encoding: 'utf8', timeout: 10000,
    input: [
      { jsonrpc: '2.0', id: 1, method: 'initialize', params: { protocolVersion: '2025-11-25', capabilities: {}, clientInfo: { name: 'desktop-fixture', version: '1' } } },
...
test('Desktop requires explicit author and a single required folder without a default', t => {
  ...
  assert.throws(() => packageClaudeDesktop(bundle), /author/); assert.equal(existsSync(bundle), false);
...
test('Desktop refuses missing/invalid binding without falling back to cwd or Claude Code', t => {
  const { root, bundle, project } = fixture(t); packageClaudeDesktop(bundle, 'Synthetic fixture author');
  for (const value of [undefined, '.', '${user_config.project_folder}', join(root, 'missing'), bundle, root]) {
    const child = run(bundle, value, project);
    assert.notEqual(child.status, 0); assert.equal(child.stdout, ''); assert.equal(child.stderr.includes(root), false);
  }
...
test('manifest-driven Desktop server isolates selected project and retains correction/history across restarts', t => {
  const { bundle, project, other } = fixture(t); const receipt = packageClaudeDesktop(bundle, 'Synthetic fixture author');
  writeFileSync(join(other, 'private.txt'), 'OTHER_PROJECT_PRIVATE_MARKER');
  const saved = result(run(bundle, project, other, { name: 'glue_checkpoint', arguments: { context: 'note.md', expectedVersion: null, markdown: 'Fictional budget 500.' } }));
  const resumed = result(run(bundle, project, other, { name: 'glue_resume', arguments: { context: 'note.md' } }));
  assert.equal(resumed.version, saved.version); assert.equal(resumed.markdown, 'Fictional budget 500.');
  result(run(bundle, project, other, { name: 'glue_checkpoint', arguments: { context: 'note.md', expectedVersion: saved.version, markdown: 'Fictional budget 350.' } }));
  const old = result(run(bundle, project, other, { name: 'glue_read', arguments: { context: 'note.md', version: saved.version } }));
  assert.equal(old.text, 'Fictional budget 500.');
  assert.equal(result(run(bundle, other, project, { name: 'glue_resume', arguments: { context: 'note.md' } })).version, null);
...
test('Desktop launcher serves when the host runtime adds its own process arguments, and never binds from them', t => {
  const { bundle, project, other } = fixture(t); packageClaudeDesktop(bundle, 'Synthetic fixture author');
...
test('Desktop bundle entry and runtime load through require(), as a host-bundled runtime may load them', t => {
  const { bundle, project } = fixture(t); packageClaudeDesktop(bundle, 'Synthetic fixture author');
  const lf = String.fromCharCode(10);
  const input = [
    { jsonrpc: '2.0', id: 1, method: 'initialize', params: { protocolVersion: '2025-11-25', capabilities: {}, clientInfo: { name: 'desktop-fixture', version: '1' } } },
    { jsonrpc: '2.0', method: 'notifications/initialized' },
    { jsonrpc: '2.0', id: 2, method: 'tools/list' },
  ].map(JSON.stringify).join(lf) + lf;
  ...
  for (const entry of ['server.mjs', 'runtime/mcp.js']) {
    const child = spawnSync(process.execPath, ['-e', 'require(process.argv[1])', join(bundle, entry)],
      { cwd: project, env: { ...process.env, GLUE_PROJECT_DIR: project }, input, encoding: 'utf8', windowsHide: true, timeout: 10000 });
    assert.doesNotMatch(child.stderr, /ERR_REQUIRE_ASYNC_MODULE/, entry);
  }
  const served = spawnSync(process.execPath, ['-e', 'require(process.argv[1])', join(bundle, 'server.mjs')],
    { cwd: project, env: { ...process.env, GLUE_PROJECT_DIR: project }, input, encoding: 'utf8', windowsHide: true, timeout: 10000 });
