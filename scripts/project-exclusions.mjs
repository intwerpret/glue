...
export function protectProjectGit(workspace, host) {
  ...
  const message = 'Glue could not verify project Git exclusions. Check Git and project file permissions, then retry. No connection was activated; existing saved work was preserved.';
  ...
  const untracked = () => {
    ...
    if (result.stdout.length) throw Error('Git already tracks files used by Glue. Exclusions cannot untrack them. Review tracking before retrying; no connection was activated and existing work was preserved.');
  };
  const ignored = () => {
  ...
  const block = '# Glue: local installation, connection and saved history\n' + rules.join('\n') + '\n';
  let protectedText;
  ...
  try {
    const current = read();
    untracked();
    const addition = current.endsWith(block) ? '' : (current && !current.endsWith('\n') ? '\n' : '') + block;
  ...
  const verify = () => {
    try { if (read() !== protectedText) throw Error(message); untracked(); if (!ignored()) throw Error(message); }
    catch (error) {
      if (error instanceof Error && error.message.startsWith('Git already tracks')) throw error;
      throw Error(message);
    }
