...
for (const file of existsSync(join(root, "dist")) ? readdirSync(join(root, "dist")) : []) {
  if (/\.(js|js\.map|d\.ts)$/.test(file) && !existsSync(join(root, "src", file.replace(/\.(js|js\.map|d\.ts)$/, ".ts")))) unlinkSync(join(root, "dist", file));
}
const result = spawnSync(process.execPath, [require.resolve("typescript/bin/tsc")], { cwd: root, stdio: "inherit" });
if (result.status !== 0) process.exit(result.status || 1);
if (sourceId !== buildInputs(root)) throw new Error("Source changed during compilation. Build again.");
