import { cpSync, existsSync, mkdirSync, readFileSync, renameSync, rmSync, mkdtempSync, writeFileSync, realpathSync } from 'node:fs';
...
export function packageHost(destination, host, author) {
  ...
  if (author !== undefined && (typeof author !== 'string' || !author.trim() || author.length > 200 || /[\x00-\x1f]/.test(author))) throw Error('Supply a valid explicitly selected author name.');
  if (host === 'claude-desktop' && !author) throw Error('Desktop packaging requires an explicitly selected author name.');
  const output = resolve(destination);
  assertUnlinked(output);
  if (existsSync(output)) throw Error('Plugin output already exists. Choose a new output directory.');
  const build = JSON.parse(readFileSync(join(source, 'dist/build.json'), 'utf8'));
  if (build.sourceId !== buildInputs(source)) throw Error('Source changed. Build before packaging.');
  ...
  try {
    ...
    const copy = (from, to) => {
      const input = join(source, from); assertUnlinked(input);
    ...
    if (host === 'claude-code') {
      ...
      json('manifest.json', {
        manifest_version: '0.3', name: 'glue', display_name: 'Glue', version, description,
        author: { name: author },
        server: { type: 'node', entry_point: 'server.mjs', mcp_config: {
          command: 'node', args: ['${__dirname}/server.mjs'], env: { GLUE_PROJECT_DIR: '${user_config.project_folder}' },
        } },
        user_config: { project_folder: { type: 'directory', title: 'Project folder',
          description: 'Select one project for Glue to save and resume. Saved work remains here when the extension is removed.', required: true, multiple: false } },
        compatibility: { platforms: ['win32', 'darwin'], runtimes: { node: '>=22' } }, tools_generated: true,
      });
    ...
    for (const file of Object.keys(dependencyFiles))
      if (file.endsWith('.js') || file.endsWith('/package.json') || file === 'package.json' || file === 'LICENSE')
        copy('node_modules/zod/' + file, 'runtime/node_modules/zod/' + file);
    if (build.sourceId !== buildInputs(source)) throw Error('Source changed while packaging. Retry after building.');
    ...
    for (const [file, hash] of Object.entries(dependencyFiles))
      if (files['runtime/node_modules/zod/' + file] !== undefined && files['runtime/node_modules/zod/' + file] !== hash)
        throw Error('Dependency bytes changed during packaging. Inspect before retrying.');
    const manifest = { format: 1, host, version, sourceId: build.sourceId, packageId: fingerprint(files), files };
    ...
    if (existsSync(stage)) {
      ...
      if (isAbsolute(child) || !child.startsWith('.glue-plugin-stage-') || child.includes('/') || child.includes('\\'))
